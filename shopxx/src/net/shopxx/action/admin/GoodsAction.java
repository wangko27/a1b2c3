...
public class GoodsAction extends BaseAdminAction {
	...
	private List<GoodsHelp> goodsHelpList;
	...
	@Resource(name = "goodsHelpServiceImpl")
	...
	public String edit() {
		...
		String relateGoods = goods.getRelateGoods();
		if (StringUtils.isNotBlank(relateGoods)) {
			List<String> retList = JsonUtil.toObject(relateGoods, new TypeReference<List<String>>() {});
			disPlayRelateGoodsList = goodsService.getGoodsListByIds(retList);
		}
	...
	public String save() {
		...
		if (goodsHelpList != null) {
			for (int i = 0; i < goodsHelpList.size(); i ++) {
				GoodsHelp goodsHelp = goodsHelpList.get(i);
				if (goodsHelp == null) {
					continue;
				}
			...
			for (Iterator<GoodsHelp> iterator = goodsHelpList.iterator(); iterator.hasNext();) {
				GoodsHelp goodsHelp = iterator.next();
				if (goodsHelp == null) {
					iterator.remove();
				}
			}
			goods.setGoodsHelpList(goodsHelpList);
		} else {
			goods.setGoodsImageList(null);
		}
		
		if (relateGoodsList != null) {
			goods.setRelateGoodsList(relateGoodsList);
		} else {
			goods.setRelateGoodsList(null);
		}
	...
	public void setGoodsHelpList(List<GoodsHelp> goodsHelpList) {
		this.goodsHelpList = goodsHelpList;
	}
	...
	public void setRelateGoodsList(List<String> relateGoodsList) {
		this.relateGoodsList = relateGoodsList;
	}

	public List<File> getGoodsHelpFileList() {
		return goodsHelpFileList;
	}

	public void setGoodsHelpFileList(List<File> goodsHelpFileList) {
		this.goodsHelpFileList = goodsHelpFileList;
	}
	...
	public List<Goods> getDisPlayRelateGoodsList() {
		return disPlayRelateGoodsList;
	}
