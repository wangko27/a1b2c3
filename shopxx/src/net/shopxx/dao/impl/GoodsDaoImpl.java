...
public class GoodsDaoImpl extends BaseDaoImpl<Goods, String> implements GoodsDao {
	...
	public List<Goods> getGoodsList(GoodsCategory goodsCategory, Date beginDate, Date endDate, Integer firstResult, Integer maxResults) {
		...
		List<Goods> retList = query.list();
		for (Goods goods : retList) {
			List<String> relateGoodsIds = goods.getRelateGoodsList();
			if (relateGoodsIds != null && relateGoodsIds.size() > 0) {
				goods.setBuildRelateGoodsList(this.getGoodsListByIds(relateGoodsIds));
			}
		}
		return retList;
	}
